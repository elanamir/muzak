...
class DecreaseVolumeIntent extends SetVolumeIntent {
  ...
  static process(squeezeserver, players, intent, session, callback) {
    try {
      ...
      console.log("Caught exception while trying to decrease volume", ex);
      callback(session.attributes, Utils.buildSpeechletResponse(intent.name, "Caught exception trying to decrease volume", null, true));
    }
  }
}
